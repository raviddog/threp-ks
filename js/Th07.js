...
var Th07 = (function() {
  ...
  var Header = Th07.Header = (function() {
    ...
    Header.prototype._read = function() {
      ...
      this.stageOffsets = new Array(7);
      for (var i = 0; i < 7; i++) {
        this.stageOffsets[i] = this._io.readU4le();
      }
      this.unknown4 = new Array(7);
      for (var i = 0; i < 7; i++) {
        this.unknown4[i] = this._io.readU4le();
      }
    }
  ...
  var ReplayHeader = Th07.ReplayHeader = (function() {
    ...
    ReplayHeader.prototype._read = function() {
      ...
      this.unknown3 = new Array(23);
      for (var i = 0; i < 23; i++) {
        this.unknown3[i] = this._io.readU4le();
      }
  ...
  Object.defineProperty(Th07.prototype, 'stages', {
    get: function() {
      ...
      this._raw__m_stages = new Array(7);
      this._m_stages = new Array(7);
      for (var i = 0; i < 7; i++) {
        switch (this.header.stageOffsets[i]) {
        case 0:
          this._raw__m_stages[i] = this._io.readBytes(40);
          var _io__raw__m_stages = new KaitaiStream(this._raw__m_stages[i]);
          this._m_stages[i] = new Dummy(_io__raw__m_stages, this, this._root);
          break;
        default:
          this._raw__m_stages[i] = this._io.readBytes(40);
          var _io__raw__m_stages = new KaitaiStream(this._raw__m_stages[i]);
          this._m_stages[i] = new Stage(_io__raw__m_stages, this, this._root);
          break;
        }
      }
      this._io.seek(_pos);
